...
const PreProgramForm = () => {
  const [responses, setResponses] = useState({});
  const [isLoading, setIsLoading] = useState(false);
  const [questions, setQuestions] = useState([
    {
      question: "I am really excited about the vision and purpose of our company.",
  ...
  const handleSubmit = async (e) => {
    ...
    if (filteredQuestions.length !== questions.length || !suggestion) {
      ...
      try {
        const response = await fetch("http://13.126.178.112:3000/preSurveyQuestions", {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span className="heading">Pre Program Survey Form</span>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: "20px" }} className="question-section">
          <p className="question-english">Q10: Do you have anything else you would like to add?</p>
          <p className="question-hindi">
            Q10: क्या आपके पास कोई और सुझाव है जिसे आप जोड़ना चाहेंगे?
          </p>
          <input
            type="input"
            value={suggestion}
